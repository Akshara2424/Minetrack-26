...
def render(project_id, project_start, milestones_df):
    st.markdown(f'''<div style="background:#161b22;border:1px solid #16a34a;border-radius:8px;padding:10px 16px;margin-bottom:1rem;">
      <span class="role-badge role-officer">👷 Officer View</span>
      <span style="color:#8b949e;font-size:0.8rem;margin-left:10px;">Entry &amp; update access only · {st.session_state.username}</span>
    </div>''', unsafe_allow_html=True)
    tab_a, tab_b = st.tabs(["✏️ Update Milestones", "➕ Add Milestone"])
