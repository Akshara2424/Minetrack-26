...
def render(milestones_df):
    ...
    st.markdown('''<div style="background:#161b22;border:1px solid #f0a500;border-radius:8px;padding:10px 16px;margin-bottom:1rem;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      <span style="color:#f0a500;font-family:'IBM Plex Mono',monospace;font-size:0.8rem;">📡 MONITORING ENGINE</span>
      <span style="color:#8b949e;font-size:0.75rem;">Simulated date: <strong style="color:#e6edf3;">March 10, 2026</strong> &nbsp;·&nbsp; 🟢 &gt;7d &nbsp;🟡 3–7d &nbsp;🔴 &lt;3d / overdue</span>
    ...
    for m in milestones:
        ...
        bn_html = f'<div style="margin-top:8px;padding:6px 10px;background:#1c1c00;border-left:3px solid #f0a500;border-radius:4px;font-size:0.75rem;color:#fbbf24;">⚙️ <strong>Bottleneck:</strong> Possible cause: {bn}</div>' if bn else ""
        notes_html = f'<div style="font-size:0.73rem;color:#8b949e;margin-top:4px;">📝 {m["notes"]}</div>' if m.get("notes") else ""
        st.markdown(f"""
        <div style="background:#161b22;border:1px solid {info['color']};border-left:4px solid {info['color']};border-radius:8px;padding:12px 16px;margin-bottom:8px;">
          <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
            <span style="font-weight:600;color:#e6edf3;">{info['emoji']} {m['name']}</span>
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
              ...
              <span style="font-size:0.75rem;color:#8b949e;font-family:'IBM Plex Mono',monospace;">{m['target_date'].strftime('%d %b %Y')}</span>
            </div>
    ...
    if run_check:
        ...
        if not to_alert:
            ...
            ch_colors = {"WhatsApp":"#25d366","SMS":"#7dd3fc","Email":"#f0a500"}
            for item in all_fired:
                cc = ch_colors.get(item["channel"],"#8b949e")
                st.markdown(
                             f"<div style='background:#0d1117;border:1px solid #30363d;border-left:3px solid {cc};border-radius:6px;padding:8px 12px;margin-bottom:5px;font-family:\"IBM Plex Mono\",monospace;font-size:0.73rem;color:#e6edf3;'><span style='color:{cc};font-weight:600;'>[{item['channel']}]</span>&nbsp;{item['message']}</div>",
    ...
    if bottlenecks:
        st.markdown("---")
        st.markdown('<div class="section-title">⚙️ Bottleneck Summary</div>', unsafe_allow_html=True)
...
def _gantt(milestones):
    ...
    tasks, color_map = [], {"green":"#16a34a","yellow":"#d97706","red":"#dc2626","complete":"#3b82f6"}
    for m in milestones:
        td = m["target_date"]
    ...
    try:
        ...
        fig = go.Figure(); fig.add_annotation(text=f"Chart unavailable: {e}",x=0.5,y=0.5,xref="paper",yref="paper",showarrow=False,font=dict(color="#dc2626"))
    today_str = TODAY.strftime("%Y-%m-%d")
    fig.add_shape(type="line",x0=today_str,x1=today_str,y0=0,y1=1,xref="x",yref="paper",line=dict(color="#f0a500",width=2,dash="dash"))
    fig.add_annotation(x=today_str,y=1.02,xref="x",yref="paper",text=f"TODAY ({TODAY.strftime('%d %b %Y')})",showarrow=False,font=dict(color="#f0a500",size=11),xanchor="left")
    fig.update_layout(paper_bgcolor="#0d1117",plot_bgcolor="#161b22",font=dict(color="#e6edf3",family="IBM Plex Mono"),margin=dict(l=10,r=10,t=30,b=10),height=320,xaxis=dict(gridcolor="#30363d",color="#8b949e"),yaxis=dict(gridcolor="#30363d",color="#e6edf3"),legend=dict(bgcolor="#161b22",bordercolor="#30363d",borderwidth=1))
    return fig